...
export class ReouvertureComponentsPage {
  createButton = element(by.id('jh-create-entity'));
  deleteButtons = element.all(by.css('jhi-reouverture div table .btn-danger'));
  title = element.all(by.css('jhi-reouverture div h2#page-heading span')).first();
...
export class ReouvertureUpdatePage {
  pageTitle = element(by.id('jhi-reouverture-heading'));
  ...
  encoursEmissionInput = element(by.id('field_encoursEmission'));
  uniteVolumeSelect = element(by.id('field_uniteVolume'));
  montantSolliciteInput = element(by.id('field_montantSollicite'));
  ...
  async setEncoursEmissionInput(encoursEmission: string): Promise<void> {
    await this.encoursEmissionInput.sendKeys(encoursEmission);
  }

  async getEncoursEmissionInput(): Promise<string> {
    return await this.encoursEmissionInput.getAttribute('value');
  }
  ...
  async setMontantSolliciteInput(montantSollicite: string): Promise<void> {
    await this.montantSolliciteInput.sendKeys(montantSollicite);
  }

  async getMontantSolliciteInput(): Promise<string> {
    return await this.montantSolliciteInput.getAttribute('value');
  }
...
export class ReouvertureDeleteDialog {
  private dialogTitle = element(by.id('jhi-delete-reouverture-heading'));
  private confirmButton = element(by.id('jhi-confirm-delete-reouverture'));
